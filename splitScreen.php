...
<!--Calls on the php variable from mySQLMagic to check if the background should be displayed via choosing if the canvas tag has a class for the css to draw-->
<?php
	if($background == 1){
		echo "<canvas id = 'myCanvas' class = 'normalCanvasBG' width = '600' height = '500' style = 'border: 5px solid 'black';>";
	}
	else{
...
<!--Game images stored inside of the canvas tag to keep them from displaying-->
...
<?php
	if($background == 1){
		echo "<canvas id = 'myCanvas2' class = 'normalCanvasBG' width = '600' height = '500' style = 'border: 5px solid 'black';>";
	}
	else{
...
<!--Calls from mySQLMagic to check whether or not to play music.-->
	
<?php
	if($bgm == 1){
		echo "<audio id = 'audio1' autoplay loop>
				<source src = 'LordUsaPianoLoop.MP3' type = 'audio/mpeg'>
			  </audio>
			  <script>
...
<script>
//Use quadTree.js to create a quad tree for collision detection. Cuts down on lag associated with lots of collision checks.
...
//Global bullet array
var playerBullets = [];
//Shot limiter. When all of these are false a player cannot shoot.
var shoot1 = false;
var shoot2 = false;
var shoot3 = false;
...
//Sprites of the player from behind
var p1HeadB;
var p1BodyB;
var p1LegsB;
...
//Checks which part to draw for each character. Values are retrieved from mySQLMagic
function checkParts(){
//head
...
//Instantiate a player object. (We do this twice) x,y are the coordinates for the lefthand canvas. x2 y2 are for the right hand canvas.
var player = {
	color: "yellow",
	x: 300,
	y: 460,
	...
	draw: function() {
	if(this.active){
	//Draws player1 from behind on the left canvas at the bottom
	...
	//Draws the front of player 1 on the right canvas on the top
		...
		//If hitbox display is turned on, draw the little square
		if(hitbox === 1){
...
//Draws objects tot he canvas by calling methods that belong to the objects themselves.
	player2.draw();
	player.draw();
//Saves off the context before mirroring the canvas. I only draw the shots onto canvas2 (the right one) while the canvas is mirrored so that I only
//need to do collision detection on one canvas rather than on two.
	context2.save();
...
player.shoot = function(){
	...
	if(shoot1){
		...
		if(shotT1 === 2){
			playerBullets.push(BulletCone({speed:1, x: bulletPosition.x, y: bulletPosition.y, P1: true}));
			shoot1=false;
			}
		if(shotT1 === 3){
		...
		if(shotT1 === 4){
			playerBullets.push(BulletRail({speed:0, x: bulletPosition.x, y: bulletPosition.y, P1: true}));
			shoot1=false;
			}
		if(shotT1 === 5){
			for(var i = 1; i<=6; i++){
				playerBullets.push(BulletHoming({fallbackSpeed: 5, x: bulletPosition.x, y: bulletPosition.y, P1: true, shotNum: i}));
			}
			shoot1=false;
			}
		if(shotT1 === 6){
			...
			for(var i = 4; i < canvas.width; i+=15){
				playerBullets.push(BulletGridlock({x: 0, y: randomY, shotNum: i, P1: true}));
			}
			for(var j = 4; j < canvas.height; j+=15){
				playerBullets.push(BulletGridlock2({x: randomX, y: canvas.height - 4, shotNum2: j, P1: true}));
			}
			shoot1=false;
			}
		if(shotT1 === 7){
			playerBullets.push(BulletMissiles({speed: 3, x:bulletPosition.x, y:canvas.height, P1: true}));
			shoot1=false;
			}
	}	
	if(shoot2){
		...
		if(shotT2 === 2){
				playerBullets.push(BulletCone({speed:2, x: bulletPosition.x, y: bulletPosition.y, P1: true}));
				shoot2=false;
				}
		if(shotT2 === 3){
		...
		if(shotT2 === 5){
				for(var i = 1; i<=6; i++){
					playerBullets.push(BulletHoming({fallbackSpeed: 7.5, x: bulletPosition.x, y: bulletPosition.y, P1: true, shotNum: i}));
				}
				shoot2=false;
				}
		if(shotT2 === 6){
		...
		if(shotT2 === 7){
				playerBullets.push(BulletMissiles({speed: 4, x:bulletPosition.x, y:canvas.height, P1: true}));
				shoot2=false;
				}
	}
	...
	if(shoot3){
		...
		if(shotT3 === 2){
				playerBullets.push(BulletCone({speed:3, x: bulletPosition.x, y: bulletPosition.y, P1: true}));
				shoot3=false;
				}
		if(shotT3 === 3){
		...
		if(shotT3 === 5){
				for(var i = 1; i<=6; i++){
					playerBullets.push(BulletHoming({fallbackSpeed: 10, x: bulletPosition.x, y: bulletPosition.y, P1: true, shotNum: i}));
				}
				shoot3=false;
				}
		if(shotT3 === 6){
				...
				for(var j = 4; j < canvas.height; j+=15){
				//y = canvas.height for player 1. Player 2 will use y = 4.
					playerBullets.push(BulletGridlock2({x: randomX, y: canvas.height - 4, shotNum2: j, P1: true}));
				}
				shoot3=false;
				}
		if(shotT3 === 7){
				playerBullets.push(BulletMissiles({speed: 5, x:bulletPosition.x, y:canvas.height, P1: true}));
				shoot3=false;
				}
	}
}
...
player2.shoot = function(){
	...
	if(shoot1P2){
		...
		if(p2Shot1 === 2){
			playerBullets.push(BulletCone({speed:1, x: bulletPosition.x, y: bulletPosition.y, P1: false}));
			shoot1P2=false;
			}
		if(p2Shot1 === 3){
		...
		if(p2Shot1 === 4){
			playerBullets.push(BulletRail({speed:0, x: bulletPosition.x, y: bulletPosition.y, P1: false}));
			shoot1P2=false;
			}
		if(p2Shot1 === 5){
			for(var i = 1; i<=6; i++){
				playerBullets.push(BulletHoming({fallbackSpeed: 5, x: bulletPosition.x, y: bulletPosition.y, P1: false, shotNum: i}));
			}
			shoot1P2=false;
			}
		if(p2Shot1 === 6){
			...
			for(var i = 4; i < canvas.width; i+=15){
				playerBullets.push(BulletGridlock({x: 0, y: randomY, shotNum: i, P1: false}));
			}
			for(var j = 4; j < canvas.height; j+=15){
				playerBullets.push(BulletGridlock2({x: randomX, y: 4, shotNum2: j, P1: false}));
			}
			shoot1P2=false;
			}
		if(p2Shot1 === 7){
			playerBullets.push(BulletMissiles({speed: 3, x:bulletPosition.x, y:0, P1: false}));
			shoot1P2=false;
			}
	}
	if(shoot2P2){
		...
		if(p2Shot2 === 2){
				playerBullets.push(BulletCone({speed:2, x: bulletPosition.x, y: bulletPosition.y, P1: false}));
				shoot2P2=false;
				}
		if(p2Shot2 === 3){
		...
		if(p2Shot2 === 5){
				for(var i = 1; i<=6; i++){
					playerBullets.push(BulletHoming({fallbackSpeed: 7.5, x: bulletPosition.x, y: bulletPosition.y, P1: false, shotNum: i}));
				}
				shoot2P2=false;
				}
		if(p2Shot2 === 6){
		...
		if(p2Shot2 === 7){
				playerBullets.push(BulletMissiles({speed: 4, x:bulletPosition.x, y:0, P1: false}));
				shoot2P2=false;
				}
	}
	...
	if(shoot3P2){
		...
		if(p2Shot3 === 2){
				playerBullets.push(BulletCone({speed:3, x: bulletPosition.x, y: bulletPosition.y, P1: false}));
				shoot3P2=false;
				}
		if(p2Shot3 === 3){
		...
		if(p2Shot3 === 5){
				for(var i = 1; i<=6; i++){
					playerBullets.push(BulletHoming({fallbackSpeed: 10, x: bulletPosition.x, y: bulletPosition.y, P1: false, shotNum: i}));
				}
				shoot3P2=false;
				}
		if(p2Shot3 === 6){
		...
		if(p2Shot3 === 7){
				playerBullets.push(BulletMissiles({speed: 5, x:bulletPosition.x, y:0, P1: false}));
				shoot3P2=false;
				}
	}
}
...
//P2	
	...
	player2.x2 = player2.x2.clamp(0, canvas2.width - player2.width);
	player2.y2 = player2.y2.clamp(canvas2.height/2 - player2.height, canvas2.height);
	
	player.x2 = player.x2.clamp(0, canvas2.width - player.width);
	player.y2 = player.y2.clamp(0, canvas2.height/2 - player.height);
}
